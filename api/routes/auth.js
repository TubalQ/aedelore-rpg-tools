...
// Protected by shared secret (INTERNAL_API_SECRET) — not for external use
router.post('/auth/oidc/jit', async (req, res) => {
    ...
    if (!provided || provided.length !== internalSecret.length ||
        !crypto.timingSafeEqual(Buffer.from(provided), Buffer.from(internalSecret))) {
