...
function updateAuthUI() {
    ...
    const mobileTabAI = document.getElementById('mobile-tab-ai');
    ...
    if (window.authToken) {
        ...
        if (mobileTabAI) mobileTabAI.style.display = '';
        ...
        if (mobileTabAI) mobileTabAI.style.display = 'none';
    }
}
...
async function deleteAccount() {
    if (!window.authToken) {
        showToast('You must be logged in to delete your account.', 'warning');
        return;
    }
    ...
    if (!await showConfirm('Are you ABSOLUTELY sure? This cannot be undone!', { confirmText: 'Delete Account', danger: true })) {
        return;
    }
