...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Intercept wiki cross-links for SPA navigation
    document.addEventListener('click', function(e) {
        const link = e.target.closest('a[href]');
        if (!link) return;
        const href = link.getAttribute('href');
        if (!href) return;
        // Match /wiki#... or wiki#... links
        const match = href.match(/^\/?(wiki#.+)/);
        if (match) {
            e.preventDefault();
            const hash = match[1].replace('wiki#', '');
...
async function loadPage(bookSlug, pageSlug) {
    ...
    try {
        ...
        // Author note (sanitized)
        if (page.author_note) {
            html += `
                <div class="wiki-author-note">
                    ${typeof DOMPurify !== 'undefined' ? DOMPurify.sanitize(page.author_note) : escapeHtml(page.author_note)}
                </div>
            `;
        }

        // Content (sanitized)
        const sanitizedContent = typeof DOMPurify !== 'undefined' ? DOMPurify.sanitize(pageContent) : escapeHtml(pageContent);
        html += `<div class="wiki-page-body">${sanitizedContent}</div>`;
