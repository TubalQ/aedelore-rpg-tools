...
function createMcpServer(token) {
    ...
    safeTool('set_attributes',
        'Set attribute values for a character. Max 5 per base stat, 10 free points total. Third eye is separate (0-3).',
        {
            ...
            third_eye: z.number().min(0).max(3).optional().describe('Third Eye value (0-3, special sense)')
        },
    ...
    safeTool('set_abilities',
        ...
        async ({ character_id, abilities }) => {
            ...
            const maxSlots = gameData.classes?.[charClass]?.startingEquipment?.abilities || 3;
            if (abilities.length > maxSlots) {
                return { content: [{ type: 'text', text: `ERROR: Class "${charClass}" only has ${maxSlots} ability slots, but ${abilities.length} abilities were provided.` }] };
            }
    ...
    safeTool('update_hp',
        ...
        {
            character_id: z.number().describe('Character ID'),
            hp: z.number().min(0).max(99).optional().describe('Set HP slider value (0-99)'),
            arcana: z.number().min(0).max(20).optional().describe('Set arcana slider value (0-20)'),
    ...
    safeTool('update_equipment_hp',
        'Update current HP for armor pieces or shield (track damage during combat)',
        {
            ...
            armor_current_hp: z.number().min(0).optional().describe('Set armor current HP'),
            armor_broken: z.boolean().optional().describe('Mark armor as broken'),
            shield_current_hp: z.number().min(0).optional().describe('Set shield current HP'),
        ...
        async ({ character_id, armor_slot, armor_current_hp, armor_broken, shield_current_hp, shield_broken }) => {
            ...
            if (armor_slot && armor_current_hp !== undefined) {
                const maxHp = parseInt(data[`armor_${armor_slot}_hp`]) || 0;
                if (maxHp > 0 && armor_current_hp > maxHp) {
                    return { content: [{ type: 'text', text: `ERROR: armor_current_hp (${armor_current_hp}) exceeds armor max HP (${maxHp}) for slot ${armor_slot}.` }] };
                }
            ...
            if (shield_current_hp !== undefined) {
                const maxShieldHp = parseInt(data.shield_hp) || 0;
                if (maxShieldHp > 0 && shield_current_hp > maxShieldHp) {
                    return { content: [{ type: 'text', text: `ERROR: shield_current_hp (${shield_current_hp}) exceeds shield max HP (${maxShieldHp}).` }] };
                }
...
app.get('/mcp/oauth/oidc-callback', async (req, res) => {
    ...
    // Validate mcp_redirect_uri against allowlist (prevent open redirect)
    if (mcp_redirect_uri) {
        try {
            const redirectHost = new URL(mcp_redirect_uri).hostname;
            if (!ALLOWED_REDIRECT_HOSTS.some(h => redirectHost === h || redirectHost.endsWith('.' + h))) {
                res.status(400).send('Invalid redirect URI');
                return;
            }
